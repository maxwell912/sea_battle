...
class drawer:
    ...
    def draw_game(scr, fields, cursor):
        drawer.draw_field(scr, fields[0], (0, fields[0].size[1] + 2), cursor, 1)
        drawer.draw_field(scr, fields[1], (0, 0), cursor, 0)
        scr.refresh()

    @staticmethod
    def draw_field(scr, field: game_field,
        ...
        x0 += 2
        drawer._draw_rectangle(scr, (x0, y0), (x0 + size[0] + 1, y0 + size[1] + 1))
        x0, y0 = x0 + 1, y0 + 1
        cur_color = 1

        def draw_symbol(s, x, y):
            drawer.draw_with_color(scr, (x0 + x, y0 + y), s, cur_color)
        ...
        for k in ((*position1, position1[0], position2[1]),
            ...
            for i in range(k[0], k[2] + 1):
                for j in range(k[1], k[3] + 1):
                    drawer.draw_with_color(scr, (i, j), ' ', 3)
